...
const (
	defaultConfigFilename = "lddldata.conf"
	defaultLogFilename    = "lddldata.log"
...
var (
	defaultHomeDir           = lddlutil.AppDataDir("lddldata", false)
	...
	defaultDBFileName = "lddldata.sqlt.db"

	defaultPGHost   = "127.0.0.1:5432"
	defaultPGUser   = "lddldata"
	defaultPGPass   = ""
	defaultPGDBName = "lddldata"
)
...
type config struct {
	...
	DBFileName         string `long:"dbfile" description:"SQLite DB file name (default is lddldata.sqlt.db)."`
...
func loadConfig() (*config, error) {
	...
	// them.
	if cfg.LddldServ == "" {
		cfg.LddldServ = defaultHost + ":" + activeNet.JSONRPCClientPort
	}
...
// netName returns the name used when referring to a Legenddigital network. TestNet3
...
func netName(chainParams *netparams.Params) string {
	...
	// TestNet2 was removed entirely for lddld 1.3.0. Compile check!
