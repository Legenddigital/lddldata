...
var user = flag.String("user", "lddld", "node RPC username")
var pass = flag.String("pass", "bananas", "node RPC password")
var cert = flag.String("cert", "lddld.cert", "node RPC TLS certificate (when notls=false)")
...
func mainCore() int {
	...
	blocks := make(map[int64]*lddlutil.Block)
	...
	for i := int64(0); i < height+1; i++ {
		...
		blocks[i] = lddlutil.NewBlock(msgBlock)
		...
		blockSummaries[i] = apitypes.BlockDataBasic{
			...
			StakeDiff:  lddlutil.Amount(header.SBits).ToCoin(),
	...
	for i := range blockSummaries {
		blockSummaries[i].PoolInfo.Value = lddlutil.Amount(poolValues[i]).ToCoin()
