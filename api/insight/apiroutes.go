...
func NewInsightContext(client *rpcclient.Client, blockData *lddlpg.ChainDBRPC, params *chaincfg.Params, memPoolData DataSourceLite, JSONIndent string) *insightApiContext {
	...
	newContext := insightApiContext{
		...
		Status: apitypes.Status{
			...
			LddldataVersion:  version.String(),
		},
	}
	return &newContext
}
...
func (c *insightApiContext) getTransaction(w http.ResponseWriter, r *http.Request) {
	...
	txsOld := []*lddljson.TxRawResult{txOld}
...
func (c *insightApiContext) getBlockSummary(w http.ResponseWriter, r *http.Request) {
	...
	blockLddld := c.BlockData.GetBlockVerboseByHash(hash, false)
	if blockLddld == nil {
		writeInsightNotFound(w, "Unable to get block")
		return
	}

	blockSummary := []*lddljson.GetBlockVerboseResult{blockLddld}
	blockInsight, err := c.LddlToInsightBlock(blockSummary)
	if err != nil {
		...
		FUNDING_TX_DUPLICATE_CHECK:
			for _, f := range addressOuts.Outpoints {
				...
				txnOutput := apitypes.AddressTxnOutput{
					...
					Amount:        lddlutil.Amount(fundingTx.Tx.TxOut[f.Index].Value).ToCoin(),
...
func (c *insightApiContext) getTransactions(w http.ResponseWriter, r *http.Request) {
	...
	if address != "" {
		// Validate Address
		_, err := lddlutil.DecodeAddress(address)
		if err != nil {
...
func (c *insightApiContext) getAddressesTxn(w http.ResponseWriter, r *http.Request) {
	...
	txsNew, err := c.LddlToInsightTxns(txsOld, noAsm, noScriptSig, noSpent)
	if err != nil {
...
func (c *insightApiContext) getAddressInfo(w http.ResponseWriter, r *http.Request) {
	...
	addressInfo := apitypes.InsightAddressInfo{
		...
		TotalReceived:            lddlutil.Amount(totalSpent + totalUnspent).ToCoin(),
		TotalSent:                lddlutil.Amount(totalSpent).ToCoin(),
		Balance:                  lddlutil.Amount(totalUnspent).ToCoin(),
		TxAppearances:            int64(confirmedTxCount),
		UnconfirmedBalance:       lddlutil.Amount(unconfirmedBalanceSat).ToCoin(),
...
func (c *insightApiContext) getEstimateFee(w http.ResponseWriter, r *http.Request) {
	...
	// A better solution would be a call to the LDDLD RPC "estimatefee" endpoint
