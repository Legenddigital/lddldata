...
// much of the time, lddldata will be using the types in lddljson, but others are
// defined here
...
// TxOut defines a Legenddigital transaction output.
type TxOut struct {
	Value     float64  `json:"value"`
	Version   uint16   `json:"version"`
...
// TxIn defines a Legenddigital transaction input.
type TxIn struct {
	// Non-witness
...
// lddljson.GetBlockVerboseResult to include the stake transaction type
type BlockDataWithTxType struct {
	*lddljson.GetBlockVerboseResult
	Votes   []TxRawWithVoteInfo
	Tickets []lddljson.TxRawResult
	Revs    []lddljson.TxRawResult
}

// TxRawWithVoteInfo adds the vote info to lddljson.TxRawResult
type TxRawWithVoteInfo struct {
	lddljson.TxRawResult
	VoteInfo VoteInfo
}

// TxRawWithTxType adds the stake transaction type to lddljson.TxRawResult
