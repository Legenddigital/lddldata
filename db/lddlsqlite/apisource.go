...
// parameters, and a status update channel. It calls lddlsqlite.NewDB to create a
...
func (db *wiredDB) GetHeader(idx int) *lddljson.GetBlockHeaderVerboseResult {
	return rpcutils.GetBlockHeaderVerbose(db.client, db.params, int64(idx))
}

func (db *wiredDB) GetBlockVerbose(idx int, verboseTx bool) *lddljson.GetBlockVerboseResult {
	return rpcutils.GetBlockVerbose(db.client, db.params, int64(idx), verboseTx)
}

func (db *wiredDB) GetBlockVerboseByHash(hash string, verboseTx bool) *lddljson.GetBlockVerboseResult {
...
func (db *wiredDB) BlockSubsidy(height int64, voters uint16) *lddljson.GetBlockSubsidyResult {
...
func makeBlockTransactions(blockVerbose *lddljson.GetBlockVerboseResult) *apitypes.BlockTransactions {
...
func (db *wiredDB) GetAllTxIn(txid string) []*apitypes.TxIn {
	...
	for i := range allTxIn {
		txIn := &apitypes.TxIn{
			...
			ValueIn:         lddlutil.Amount(allTxIn0[i].ValueIn).ToCoin(),
...
func (db *wiredDB) GetAllTxOut(txid string) []*apitypes.TxOut {
	...
	for i := range allTxOut {
		...
		txOut := &apitypes.TxOut{
			Value:     lddlutil.Amount(allTxOut0[i].Value).ToCoin(),
...
func (db *wiredDB) DecodeRawTransaction(txhex string) (*lddljson.TxRawResult, error) {
...
func (db *wiredDB) getRawTransaction(txid string) (*apitypes.Tx, string) {
	...
	tx.Vin = make([]lddljson.Vin, len(txraw.Vin))
...
// GetVoteVersionInfo requests stake version info from the lddld RPC server
func (db *wiredDB) GetVoteVersionInfo(ver uint32) (*lddljson.GetVoteInfoResult, error) {
...
func (db *wiredDB) GetStakeVersions(txHash string, count int32) (*lddljson.GetStakeVersionsResult, error) {
...
func (db *wiredDB) GetStakeVersionsLatest() (*lddljson.StakeVersions, error) {
...
// on the stake version with which lddldata is compiled with (chaincfg.Params),
...
func (db *wiredDB) GetFeeInfo(idx int) *lddljson.FeeInfoBlock {
...
func (db *wiredDB) GetAddressTransactionsWithSkip(addr string, count, skip int) *apitypes.Address {
	address, err := lddlutil.DecodeAddress(addr)
	if err != nil {
...
func (db *wiredDB) GetAddressTransactionsRawWithSkip(addr string, count int, skip int) []*apitypes.AddressTxRaw {
	...
	for i := range txs {
		...
		tx.Vin = make([]lddljson.VinPrevOut, len(txs[i].Vin))
...
func makeExplorerBlockBasic(data *lddljson.GetBlockVerboseResult) *explorer.BlockBasic {
...
func makeExplorerTxBasic(data lddljson.TxRawResult, msgTx *wire.MsgTx, params *chaincfg.Params) *explorer.TxBasic {
...
func makeExplorerAddressTx(data *lddljson.SearchRawTransactionsResult, address string) *explorer.AddressTx {
...
func (db *wiredDB) GetExplorerBlock(hash string) *explorer.BlockInfo {
	...
	getTotalFee := func(txs []*explorer.TxBasic) (total lddlutil.Amount) {
		for _, tx := range txs {
			total += tx.Fee
		}
		return
	}
	getTotalSent := func(txs []*explorer.TxBasic) (total lddlutil.Amount) {
		for _, tx := range txs {
			amt, err := lddlutil.NewAmount(tx.Total)
			if err != nil {
				continue
			}
			total += amt
...
func (db *wiredDB) GetExplorerTx(txid string) *explorer.TxInfo {
	...
	for i, vin := range txraw.Vin {
		var addresses []string
		// ValueIn is a temporary fix until amountIn is correct from lddld call
		var ValueIn lddlutil.Amount
		if !(vin.IsCoinBase() || (vin.IsStakeBase() && i == 0)) {
			var addrs []string
			...
			ValueIn, _ = lddlutil.NewAmount(vin.AmountIn)
		}
...
func (db *wiredDB) GetExplorerAddress(address string, count, offset int64) *explorer.AddressInfo {
	...
	var totalreceived, totalsent lddlutil.Amount
		...
		for _, y := range tx.Vout {
			if len(y.ScriptPubKey.Addresses) != 0 {
				if address == y.ScriptPubKey.Addresses[0] {
					t, _ := lddlutil.NewAmount(y.Value)
					if t > 0 {
						totalreceived += t
					}
					numReceiving++
		...
		for _, u := range tx.Vin {
			if u.PrevOut != nil && len(u.PrevOut.Addresses) != 0 {
				if address == u.PrevOut.Addresses[0] {
					t, _ := lddlutil.NewAmount(*u.AmountIn)
					if t > 0 {
						totalsent += t
					}
					numSpending++
...
func ValidateNetworkAddress(address lddlutil.Address, p *chaincfg.Params) bool {
...
func (db *wiredDB) UnconfirmedTxnsForAddress(address string) (*txhelpers.AddressOutpoints, int64, error) {
	...
	mempoolTxns, err := db.client.GetRawMempoolVerbose(lddljson.GRMAll)
	if err != nil {
	...
	for hash, tx := range mempoolTxns {
		// Transaction details from lddld
...
func (db *wiredDB) GetMempool() []explorer.MempoolTx {
	mempooltxs, err := db.client.GetRawMempoolVerbose(lddljson.GRMAll)
	if err != nil {
