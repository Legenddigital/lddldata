...
// (hash:index).
// Update Vin due to LDDLD AMOUNTIN - START - DO NOT MERGE CHANGES IF LDDLD FIXED
...
func RetrieveAddressTxnOutputWithTransaction(db *sql.DB, address string, currentBlockHeight int64) ([]apitypes.AddressTxnOutput, error) {
	...
	for rows.Next() {
		...
		txnOutput.Amount = lddlutil.Amount(atoms).ToCoin()
...
func RetrieveTicketsPriceByHeight(db *sql.DB, val int64) (*dbtypes.ChartsData, error) {
	...
	for rows.Next() {
		...
		priceCoin := lddlutil.Amount(price).ToCoin()
...
func retrieveCoinSupply(db *sql.DB) (*dbtypes.ChartsData, error) {
	...
	for rows.Next() {
		...
		sum += lddlutil.Amount(value).ToCoin()
...
func InsertTickets(db *sql.DB, dbTxns []*dbtypes.Tx, txDbIDs []uint64, checked bool) ([]uint64, []*dbtypes.Tx, error) {
	...
	for i, tx := range ticketTx {
		...
		price := lddlutil.Amount(tx.Vouts[0].Value).ToCoin()
		fee := lddlutil.Amount(tx.Fees).ToCoin()
		isSplit := tx.NumVin > 1
...
func InsertVotes(db *sql.DB, dbTxns []*dbtypes.Tx, _ /*txDbIDs*/ []uint64,
	...
	for i, tx := range voteTxs {
		...
		stakeSubmissionAmount := lddlutil.Amount(msgTx.TxIn[1].ValueIn).ToCoin()
		...
		voteReward := lddlutil.Amount(msgTx.TxIn[0].ValueIn).ToCoin()
