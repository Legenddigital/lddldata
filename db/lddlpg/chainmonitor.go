...
func (p *ChainMonitor) switchToSideChain() (int32, *chainhash.Hash, error) {
	...
	defer func(start time.Time) {
		log.Infof("lddlpg: switchToSideChain completed in %v", time.Since(start))
	}(time.Now())
	...
	prevBlock := lddlutil.NewBlock(prevMsgBlock)
	...
	for i := range p.sideChain {
		...
		if !found || msgBlock.BlockHash() != p.sideChain[i] {
			log.Debugf("block %v not found in stakedb cache, fetching from lddld", p.sideChain[i])
			// Request MsgBlock from lddld
