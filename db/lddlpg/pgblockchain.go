...
import (
	...
	"github.com/Legenddigital/lddldata/db/lddlpg/internal"
...
// NewChainDB constructs a ChainDB for the given connection and Legenddigital network
...
// DropTables drops (deletes) all of the known lddldata tables.
...
func (pgb *ChainDB) AddressHistory(address string, N, offset int64,
	...
	log.Infof("%s: %d spent totalling %f LDDL, %d unspent totalling %f LDDL",
		address, balanceInfo.NumSpent, lddlutil.Amount(balanceInfo.TotalSpent).ToCoin(),
		balanceInfo.NumUnspent, lddlutil.Amount(balanceInfo.TotalUnspent).ToCoin())
...
func (pgb *ChainDB) FillAddressTransactions(addrInfo *explorer.AddressInfo) error {
	...
	for _, txn := range addrInfo.Transactions {
		...
		txn.Total = lddlutil.Amount(dbTx.Sent).ToCoin()
...
func (pgb *ChainDB) AddressTotals(address string) (*apitypes.AddressTotals, error) {
	...
	return &apitypes.AddressTotals{
		...
		CoinsSpent:   lddlutil.Amount(ab.TotalSpent).ToCoin(),
		CoinsUnspent: lddlutil.Amount(ab.TotalUnspent).ToCoin(),
	}, nil
}
...
func (pgb *ChainDB) DeleteDuplicates() error {
	...
	if numVinsRemoved, err = pgb.DeleteDuplicateVins(); err != nil {
		return fmt.Errorf("lddlpg.DeleteDuplicateVins failed: %v", err)
	}
	...
	if numVoutsRemoved, err = pgb.DeleteDuplicateVouts(); err != nil {
		return fmt.Errorf("lddlpg.DeleteDuplicateVouts failed: %v", err)
	}
	...
	if numTxnsRemoved, err = pgb.DeleteDuplicateTxns(); err != nil {
		return fmt.Errorf("lddlpg.DeleteDuplicateTxns failed: %v", err)
	}
...
func (pgb *ChainDB) DeleteDuplicatesRecovery() error {
	...
	if numTxnsRemoved, err = pgb.DeleteDuplicateTickets(); err != nil {
		return fmt.Errorf("lddlpg.DeleteDuplicateTickets failed: %v", err)
	}
	...
	if numTxnsRemoved, err = pgb.DeleteDuplicateVotes(); err != nil {
		return fmt.Errorf("lddlpg.DeleteDuplicateVotes failed: %v", err)
	}
	...
	if numTxnsRemoved, err = pgb.DeleteDuplicateMisses(); err != nil {
		return fmt.Errorf("lddlpg.DeleteDuplicateMisses failed: %v", err)
	}
