...
type BlockGetter interface {
	...
	BestBlock() (*lddlutil.Block, error)
	Block(chainhash.Hash) (*lddlutil.Block, error)
...
// directly from lddld via RPC and subsequently update the internal block cache
// with the retrieved block.
type MasterBlockGetter interface {
	BlockGetter
	UpdateToBestBlock() (*lddlutil.Block, error)
	UpdateToNextBlock() (*lddlutil.Block, error)
	UpdateToBlock(height int64) (*lddlutil.Block, error)
}
...
type BlockGate struct {
	...
	blockWithHash map[chainhash.Hash]*lddlutil.Block
...
func NewBlockGate(client *rpcclient.Client, capacity int) *BlockGate {
	return &BlockGate{
		...
		blockWithHash: make(map[chainhash.Hash]*lddlutil.Block),
...
// NodeHeight gets the chain height from lddld.
...
func (g *BlockGate) BestBlock() (*lddlutil.Block, error) {
	g.RLock()
	defer g.RUnlock()
	var err error
	hash, ok := g.hashAtHeight[g.height]
...
func (g *BlockGate) Block(hash chainhash.Hash) (*lddlutil.Block, error) {
	g.RLock()
	defer g.RUnlock()
	var err error
	block, ok := g.blockWithHash[hash]
...
func (g *BlockGate) UpdateToBestBlock() (*lddlutil.Block, error) {
	_, height, err := g.client.GetBestBlock()
	if err != nil {
...
// updates the cache.
func (g *BlockGate) UpdateToNextBlock() (*lddlutil.Block, error) {
	g.Lock()
	height := g.height + 1
	g.Unlock()
	return g.UpdateToBlock(height)
...
// lddld and stores it in cache.
func (g *BlockGate) UpdateToBlock(height int64) (*lddlutil.Block, error) {
	g.Lock()
	defer g.Unlock()
	return g.updateToBlock(height)
}
