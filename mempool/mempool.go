...
func (p *mempoolMonitor) TxHandler(client *rpcclient.Client) {
	...
	for {
		...
		case s, ok := <-p.newTxHash:
			...
			if err != nil {
				log.Errorf("Failed to get transaction (do you have --txindex with lddld?) %v: %v",
					s.Hash.String(), err)
				continue
			}
			...
			//s.Tree() == lddlutil.TxTreeRegular
			// See lddld/blockchain/stake/staketx.go for information about
			...
			case stake.TxTypeSStx:
				...
				log.Tracef("Received ticket purchase %v, price %v",
					tx.Hash(), lddlutil.Amount(price).ToCoin())
...
type MempoolData struct {
	...
	NumVotes          uint32
	NewTickets        uint32
	Ticketfees        *lddljson.TicketFeeInfoResult
...
func (t *mempoolDataCollector) Collect() (*MempoolData, error) {
	...
	mempoolTickets, err := c.GetRawMempoolVerbose(lddljson.GRMTickets)
	if err != nil {
		return nil, err
	}

	mempoolVotes, err := c.GetRawMempoolVerbose(lddljson.GRMVotes)
	if err != nil {
		return nil, err
	}
