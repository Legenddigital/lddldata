...
type TxGetter struct {
	txLookup map[chainhash.Hash]*lddlutil.Tx
}

func (t TxGetter) GetRawTransaction(txHash *chainhash.Hash) (*lddlutil.Tx, error) {
...
func LoadTestBlockAndSSTX(t *testing.T) (*lddlutil.Block, []*lddlutil.Tx) {
	// Load block data
	...
	block, err := lddlutil.NewBlockFromReader(blockTestFile)
	if err != nil {
	...
	allTxRead := make([]*lddlutil.Tx, numSSTX)
	for i := range allTxRead {
		...
		allTxRead[i], err = lddlutil.NewTxFromReader(txFile)
		if err != nil {
			t.Fatal(err)
		}
...
func TestFeeRateInfoBlock(t *testing.T) {
	...
	fibExpected := lddljson.FeeInfoBlock{
		Height: 138883,
		Number: 20,
...
func TxToWriter(tx *lddlutil.Tx, w io.Writer) error {
	msgTx := tx.MsgTx()
...
func ConnectNodeRPC(host, user, pass, cert string, disableTLS bool) (*rpcclient.Client, semver.Semver, error) {
	...
	lddldClient, err := rpcclient.New(connCfgDaemon, nil)
	if err != nil {
