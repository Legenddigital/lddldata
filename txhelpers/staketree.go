...
import (
	...
	_ "github.com/Legenddigital/lddld/database/ffldb" // init the ffldb driver
...
func BuildStakeTree(blocks map[int64]*lddlutil.Block, netParams *chaincfg.Params,
...
func TicketsInBlock(bl *lddlutil.Block) ([]chainhash.Hash, []*wire.MsgTx) {
...
func TicketTxnsInBlock(bl *lddlutil.Block) ([]chainhash.Hash, []*lddlutil.Tx) {
	tickets := make([]chainhash.Hash, 0)
	ticketTxns := make([]*lddlutil.Tx, 0)
	for _, stx := range bl.STransactions() {
		if stake.DetermineTxType(stx.MsgTx()) == stake.TxTypeSStx {
			h := stx.Hash()
			tickets = append(tickets, *h)
...
func TicketsSpentInBlock(bl *lddlutil.Block) []chainhash.Hash {
	tickets := make([]chainhash.Hash, 0)
	for _, stx := range bl.STransactions() {
...
func VotesInBlock(bl *lddlutil.Block) []chainhash.Hash {
...
func RevokedTicketsInBlock(bl *lddlutil.Block) []chainhash.Hash {
	tickets := make([]chainhash.Hash, 0)
	for _, stx := range bl.STransactions() {
