...
type BlockWatchedTx struct {
	BlockHeight   int64
	TxsForAddress map[string][]*lddlutil.Tx
}
...
// TxhashInSlice searches a slice of *lddlutil.Tx for a transaction with the hash
// txHash. If found, it returns the corresponding *Tx, otherwise nil.
func TxhashInSlice(txs []*lddlutil.Tx, txHash *chainhash.Hash) *lddlutil.Tx {
	if len(txs) < 1 {
		return nil
	}
...
func IncludesStakeTx(txHash *chainhash.Hash, block *lddlutil.Block) (int, int8) {
...
func IncludesTx(txHash *chainhash.Hash, block *lddlutil.Block) (int, int8) {
...
func BlockConsumesOutpointWithAddresses(block *lddlutil.Block, addrs map[string]TxAction,
	c RawTransactionGetter, params *chaincfg.Params) map[string][]*lddlutil.Tx {
	addrMap := make(map[string][]*lddlutil.Tx)

	checkForOutpointAddr := func(blockTxs []*lddlutil.Tx) {
					...
					for _, txAddr := range txAddrs {
						...
						if _, ok := addrs[addrstr]; ok {
							if addrMap[addrstr] == nil {
								addrMap[addrstr] = make([]*lddlutil.Tx, 0)
							}
...
// specified addresses, and creates a map of addresses to a slice of lddlutil.Tx
// involving the address.
func BlockReceivesToAddresses(block *lddlutil.Block, addrs map[string]TxAction,
	params *chaincfg.Params) map[string][]*lddlutil.Tx {
	addrMap := make(map[string][]*lddlutil.Tx)

	checkForAddrOut := func(blockTxs []*lddlutil.Tx) {
		for _, tx := range blockTxs {
				...
				for _, txAddr := range txOutAddrs {
					...
					if _, ok := addrs[addrstr]; ok {
						if _, gotSlice := addrMap[addrstr]; !gotSlice {
							addrMap[addrstr] = make([]*lddlutil.Tx, 0) // nil
						}
...
func MedianAmount(s []lddlutil.Amount) lddlutil.Amount {
	if len(s) == 0 {
		return 0
	}

	sort.Sort(lddlutil.AmountSorter(s))
...
// MedianCoin gets the median LDDL from a slice of float64s
...
func SSTXInBlock(block *lddlutil.Block) []*lddlutil.Tx {
...
func SSGenVoteBlockValid(msgTx *wire.MsgTx) (BlockValidation, uint16, error) {
	...
	blockValid := BlockValidation{
		...
		Validity: lddlutil.IsFlagSet16(ssGenVoteBits, lddlutil.BlockValid),
	}
...
// specified block.
func FeeInfoBlock(block *lddlutil.Block) *lddljson.FeeInfoBlock {
	feeInfo := new(lddljson.FeeInfoBlock)
	...
	for it, msgTx := range sstxMsgTxns {
		...
		fee := lddlutil.Amount(amtIn - amtOut).ToCoin()
		if fee < minFee {
			minFee = fee
		}
		if fee > maxFee {
...
func FeeRateInfoBlock(block *lddlutil.Block) *lddljson.FeeInfoBlock {
	feeInfo := new(lddljson.FeeInfoBlock)
	_, sstxMsgTxns := TicketsInBlock(block)
	...
	for it, msgTx := range sstxMsgTxns {
		...
		fee := lddlutil.Amount(1000*(amtIn-amtOut)).ToCoin() / float64(msgTx.SerializeSize())
		if fee < minFee {
			minFee = fee
		}
		if fee > maxFee {
...
func TxFee(msgTx *wire.MsgTx) lddlutil.Amount {
	...
	return lddlutil.Amount(amtIn - amtOut)
}

// TxFeeRate computes and returns the fee rate in LDDL/KB for a given tx
func TxFeeRate(msgTx *wire.MsgTx) (lddlutil.Amount, lddlutil.Amount) {
	var amtIn int64
	for iv := range msgTx.TxIn {
		amtIn += msgTx.TxIn[iv].ValueIn
	}
...
func TotalVout(vouts []lddljson.Vout) lddlutil.Amount {
	var total lddlutil.Amount
	for _, v := range vouts {
		a, err := lddlutil.NewAmount(v.Value)
		if err != nil {
			continue
		}
		total += a
