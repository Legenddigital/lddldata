...
Presently the lddldata executable, it's config file, logs, data files, and web
...
Then edit lddldata.conf with your lddld RPC settings. See the output of `lddldata
...
indexes. This will be done automatically by lddldata, but the PostgreSQL tables
...
- Set your PostgreSQL credentials and host in both `./cmd/rebuilddb2/rebuilddb2.conf` and `./lddldata.conf`.
...
lddldata is run, but they will be done concurrently. On subsequent launches, only
blocks new to lddldata are scanned.

```bash
./lddldata    # don't forget to configure lddldata.conf in this folder
```

The "public" and "views" folders *must* be in the same folder as the `lddldata`
executable.
...
PostgreSQL backend (and the expanded functionality), lddldata may be started with
...
to keep in mind that the mempool in your node (lddld) is not likely to be the
